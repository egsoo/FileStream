    ...
    try:
        ...
        if Telegram.ULOG_CHANNEL:
            await bot.send_message(
                ...
                text=(
                    f"⚠️ <b>FloodWait</b> of <code>{e.value}s</code>\n"
    ...
    try:
        ...
        if Telegram.ULOG_CHANNEL:
            await bot.send_message(
                chat_id=Telegram.ULOG_CHANNEL,
                text=(
                    f"⚠️ <b>FloodWait</b> of <code>{w.value}s</code>\n"
                    f"Channel: <b>{message.chat.title}</b> (<code>{message.chat.id}</code>)"
                ),
                disable_web_page_preview=True,
                parse_mode=ParseMode.HTML
            )
        ...
        if Telegram.ULOG_CHANNEL:
            await bot.send_message(
                chat_id=Telegram.ULOG_CHANNEL,
                text=f"❌ <b>Error in channel handler:</b>\n<code>{e}</code>",
                disable_web_page_preview=True,
                parse_mode=ParseMode.HTML
            )
